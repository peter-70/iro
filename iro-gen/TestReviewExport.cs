...
public static class TestReviewExport
{
    public static string Create(IReadOnlyList<TestRunRow> rows, double tolerance, bool onlyProblems)
    {
        ...
        text.AppendLine("Entwicklungsdiagnose, keine Abnahme. Nominale Materialabstände vor Störungen sind keine geprüften Bild-Sollwerte. Abweisung kann richtig sein. Fehlerfälle unten sind Beispiele, keine vollständige Einzelfallliste.\n");
        foreach (var verdict in Enum.GetValues<ReviewVerdict>())
            text.AppendLine($"- {verdict}: {rows.Count(r => r.Verdict == verdict)}");
        ...
        foreach (var group in rows.GroupBy(r => new { r.CaseName, r.Scene, r.Disturbances, r.OptionsText, r.AnalyzerVersion, r.GeneratorVersion, r.AnalysisProfile }))
        {
            ...
            text.AppendLine($"Bilder: {group.Count()}; vollständig/genau: {group.Count(r => r.Verdict == ReviewVerdict.Erreicht)}; abweichend: {group.Count(r => r.Verdict == ReviewVerdict.FalscherMesswert)}; teilweise: {group.Count(r => r.Verdict == ReviewVerdict.Teilweise)}; abgewiesen: {group.Count(r => r.Verdict == ReviewVerdict.Abgewiesen)}; Lesefehler: {group.Count(r => r.Verdict == ReviewVerdict.Fehler)}.");
            ...
            var examples = group.GroupBy(r => r.Verdict).Select(g => g.OrderByDescending(r => r.MaxDeviation ?? -1).ThenBy(r => r.Measured).First()).OrderByDescending(r => r.Verdict == ReviewVerdict.FalscherMesswert)
