...
public sealed record TestPlan(int FormatVersion, string Kind, string Name, GeneratorOptions Defaults, List<TestPlanCase> Cases)
{
    ...
    {
        if (FormatVersion != 1 || Kind != "irogen-test-plan" || string.IsNullOrWhiteSpace(Name)
        ...
        foreach (var entry in Cases)
        {
            ...
            var options = merged.Deserialize<GeneratorOptions>(SceneExport.JsonOptions)! with { TestCaseName = entry.Name };
            options.Validate();
