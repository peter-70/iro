...
internal sealed record Detection(IReadOnlyList<PixelRect> Fields, bool Ambiguous, bool HadSmallRegions, string Orientation, IReadOnlyList<PixelRect>? BorderRegions = null, IReadOnlyList<PixelRect>? InconsistentFields = null);
    ...
    public static Detection Detect(RgbFrame image, AnalysisOptions options, CancellationToken token)
    {
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                { var p = image.GetPixel(xx, yy); rr += p.R; gg += p.G; bb += p.B; n++; }
        ...
        if (groups.Count == 1)
        {
            ...
            return new(group.Fields.OrderBy(r => group.Vertical ? r.Y : r.X).ToArray(), false, small,
                group.Vertical ? "vertical" : "horizontal", borderRegions, inconsistent);
        }
